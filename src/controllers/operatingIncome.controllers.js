...
export const getData = async (req, res) => {
    ...
    const operatingIncome = [
        "OperatingIncomeLoss",
        "IncomeLossFromContinuingOperationsBeforeIncomeTaxesExtraordinary", "ItemsNoncontrollingInterest",
        "IncomeLossFromContinuingOperationsBeforeIncomeTaxesMinority", "InterestAndIncomeLossFromEquityMethodInvestments",
    ...
    try {
        ...
        const [rows] = await connection.execute(
            `WITH ranked_data AS (
                ...
                WHERE n.tag IN (${operatingIncome.map(() => '?').join(', ')})
                    AND ct.cik = ?
                    AND s.form = '10-K'
